...
- **`AbortSignal`** — a deadline on every call, handed to the participant and
  ...
  cancellation.
- **`modelContext.requestUserInteraction`, feature-detected** — the draft's own
  way for a tool to say a person is needed. Chrome 151 does not implement it, so
  Concord measures that on `/native.html` and uses it wherever it exists: when a
  guarantee is explained, the host is asked to bring the person to the page, and
  the agent is told whether anybody actually was. It is a **second door onto the
  same accept and deliberately not a lock** — it grants nothing, and a test
  gives it a host that approves every request and requires the surface to stay
  exactly as it was. If asking for attention could produce consent, an agent
  that can ask could manufacture it, and the central claim would be false.

- **A formal model of the protocol** — `spec/` states the commitment machine
  once, checks nine safety invariants over all 855 of its reachable states, and
  **generates** `spec/Concord.tla` from the same declaration, so the TLA+ module
  cannot drift from what is verified. One of those invariants is
  `CommitToolIffUnspentAcceptance`, and a test hands every reachable state to
  the shipped `desiredNames()` and requires it to agree — so the proof is about
  this coordinator rather than about a document describing one.
...
163 assertions over the protocol with no browser · 855 states of the commitment
machine model-checked against 9 safety invariants · 4,164 ladder configurations
checked exhaustively rather than sampled · 97% mutation score over every one of 116 mutants, not a sample, with the four survivors documented as equivalent · 14 forged
receipts rejected, fireable at
[/attack.html](https://concord-coordinator.vercel.app/attack.html), plus **all
180 receipts a malicious coordinator can build** from one commitment's
materials, judged against a rule rather than a list · 7 browser suites against
real origins · 5 participants at conformance level L3 · 8 independent HTTPS
...
Every one of those is regenerated by `npm run check` and committed under
[`evidence/`](evidence/), so a number that stops being true stops being
printed.
