...
> ### ▸ [**The five-minute path**](https://concord-coordinator.vercel.app/judge.html) — five claims, each with the link that shows it, and nothing to type.
>
> Or the two that matter most, directly:
> [**watch the commit tool appear when a person accepts**](https://concord-coordinator.vercel.app/?ask=Book%20me%20London%20for%20three%20nights%20%E2%80%94%20flight%2C%20hotel%20and%20the%20visa%20fee.)
...
- **[`concord/exhaustive.test.mjs`](concord/exhaustive.test.mjs)** enumerates
  **every** configuration of up to three participants — each reservable,
  ...
  dependency including the cycles — and checks the ladder's safety property over
  all 4,164 of them. Not a sample: the state count is asserted, so growing the
  model forces somebody to decide whether the new states are still covered. It
  is what would otherwise be a TLA+ model, written against the real `plan()`
